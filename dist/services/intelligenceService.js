...
class IntelligenceService {
    static TARGET_LOCATION = "Atimonan, Quezon";
    static NEARBY_LOCATIONS = ["Pagbilao", "Plaridel", "Gumaca"];
    /**
     * Generates location-specific fuel intelligence for Atimonan.
     */
    static async getFuelIntelligence(fuelType) {
        // 1. Fetch data for Atimonan
        const directRecords = await FinalPublishedFuelPrice_1.FinalPublishedFuelPrice.find({
            fuelType,
            $or: [
        ...
        const userReports = await FuelPriceLog_1.default.find({
            fuelType,
            source: "USER",
            isOutlier: false,
        ...
        if (prices.length === 0) {
            const nearbyRecords = await FinalPublishedFuelPrice_1.FinalPublishedFuelPrice.find({
                fuelType,
        ...
        const oldRecords = await FinalPublishedFuelPrice_1.FinalPublishedFuelPrice.find({
            fuelType,
            city: /atimonan/i,
     ...
     */
    static async handleChatQuery(query) {
        const q = query.toLowerCase();
        let fuelType = "Gasoline";
        if (q.includes("diesel"))
            fuelType = "Diesel";
        else if (q.includes("kerosene"))
            fuelType = "Kerosene";
