...
exports.inferDoeDocumentDateFromText = inferDoeDocumentDateFromText;
exports.inferDoeDocumentDateFromLabel = inferDoeDocumentDateFromLabel;
exports.inferDoeDocumentDateFromUrl = inferDoeDocumentDateFromUrl;
const MONTH_LOOKUP = {
    jan: 1,
    january: 1,
    feb: 2,
...
function inferDoeDocumentDateFromText(text) {
    const monthPattern = buildMonthPattern();
    // Example: "For the week of March 10-16, 2026"
    const sameMonthWeek = new RegExp(`for the week of\\s+${monthPattern}\\s+([0-9]{1,2})\\s*(?:to|-)\\s*([0-9]{1,2}),?\\s+(20[0-9]{2})`, "i");
    ...
    if (sameMonthMatch) {
        ...
        if (month && year)
            return toUtcDate(year, month, endDay);
    }
    // Example: "For the week of February 24 to March 2, 2026"
    const crossMonthWeek = new RegExp(`for the week of\\s+${monthPattern}\\s+([0-9]{1,2})\\s*(?:to|-)\\s*${monthPattern}\\s+([0-9]{1,2}),?\\s+(20[0-9]{2})`, "i");
    ...
    if (crossMonthMatch) {
        ...
        if (endMonth && year)
            return toUtcDate(year, endMonth, endDay);
    }
...
function inferDoeDocumentDateFromLabel(label, fallbackYear) {
    const monthPattern = buildMonthPattern();
    ...
    if (sameMonthMatch && fallbackYear) {
        ...
        const endDay = Number(sameMonthMatch[3]);
        if (month)
            return toUtcDate(fallbackYear, month, endDay);
    }
    // Example: "February 24 to March 2"
    const crossMonthRange = new RegExp(`^\\s*${monthPattern}\\s+([0-9]{1,2})\\s*(?:to|-)\\s*${monthPattern}\\s+([0-9]{1,2})\\s*$`, "i");
    const crossMonthMatch = label.match(crossMonthRange);
    if (crossMonthMatch && fallbackYear) {
        const endMonth = normalizeMonthToken(crossMonthMatch[3]);
        const endDay = Number(crossMonthMatch[4]);
        if (endMonth)
            return toUtcDate(fallbackYear, endMonth, endDay);
    }
    return null;
}
function inferDoeDocumentDateFromUrl(url) {
