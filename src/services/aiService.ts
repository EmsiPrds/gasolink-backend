...
import type { GlobalPriceDoc } from "../models/GlobalPrice";
import { searchWeb, type SearchResult } from "./searchService";

let groqInstance: Groq | null = null;

function getGroqClient(): Groq {
  if (groqInstance) return groqInstance;
  if (!env.GROQ_API_KEY || env.GROQ_API_KEY.includes("replace_me")) {
...
async function callOpenRouter(params: any, retries = 2): Promise<any> {
...
async function callOpenAI(params: any, retries = 2): Promise<any> {
...
async function callAiProvider(params: any): Promise<any> {
  const hasOpenRouter = env.OPENROUTER_API_KEY && !env.OPENROUTER_API_KEY.includes("replace_me");
  const hasGroq = env.GROQ_API_KEY && !env.GROQ_API_KEY.includes("replace_me");
  const hasOpenAI = env.OPENAI_API_KEY && !env.OPENAI_API_KEY.includes("replace_me");
  ...
  if (env.AI_PROVIDER === "openrouter" && hasOpenRouter) {
    try {
...
export type AiPriceEstimationResult = {
...
export type AiDiscoveryResult = {
  links: Array<{
    url: string;
    label: string;
    category: "PriceAdjustment" | "NCR" | "Luzon" | "Visayas" | "Mindanao" | "Unknown";
    isLatest: boolean;
  }>;
};

export async function extractFuelDataWithAi(text: string): Promise<AiExtractionResult | null> {
  const prompt = `
...
export async function validateUserReportWithAi(
  report: { fuelType: string; price: number; location: string },
  context: { regionalAverage: number; lastOfficialPrice: number }
...
`;
  ...
  try {
    ...
    const content = response.choices[0]?.message?.content;
    if (!content) return { isValid: false, confidence: 0, reasoning: "AI failed to respond" };
...
async function callGroqWithFallback(params: any, retries = 2): Promise<any> {
  let lastError: any;
  const models = [env.GROQ_MODEL, env.GROQ_FALLBACK_MODEL];
...
export async function refinePriceWithAi(
  fuelType: string,
  region: string,
  candidates: NormalizedFuelRecordDoc[],
  globalPrices: GlobalPriceDoc[],
): Promise<AiPriceEstimationResult | null> {
  const candidateSummary = candidates
    .map(
      (c) =>
        `- ${c.sourceType} (${c.sourceName}): Price=${c.pricePerLiter ?? "N/A"}, Change=${
          c.priceChange ?? "N/A"
        }, Confidence=${c.confidenceScore}, Date=${c.scrapedAt.toISOString()}`,
    )
    .join("\n");
...
`;
  ...
  try {
    ...
    return JSON.parse(content) as AiPriceEstimationResult;
  } catch (error) {
...
export async function discoverLatestLinksWithAi(
  sourceUrl: string,
  rawHtml: string,
