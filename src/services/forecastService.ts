...
async function getPctChange(type: "Brent" | "WTI" | "USDPHP", days: number) {
  const latest = await GlobalPrice.findOne({ type }).sort({ timestamp: -1 }).lean();
  if (!latest) return { latest: null as any, pct: 0 };

  const from = new Date(latest.timestamp);
  from.setDate(from.getDate() - days);

  const past = await GlobalPrice.findOne({ type, timestamp: { $lte: from } })
    .sort({ timestamp: -1 })
    .lean();
  if (!past) return { latest, pct: 0 };
...
export async function buildForecast(region: string) {
  const [{ pct: brent7dPct }, { pct: wti7dPct }, { pct: usdphp7dPct }] =
    await Promise.all([getPctChange("Brent", 7), getPctChange("WTI", 7), getPctChange("USDPHP", 7)]);

  const score = brent7dPct * 0.5 + wti7dPct * 0.3 + usdphp7dPct * 0.2;
  const estimated = clamp(score * 0.15, -3, 3);

  const latestPh = await FuelPricePH.aggregate([
    { $match: { region } },
    { $sort: { updatedAt: -1 } },
  ...
  const cards: ForecastCard[] = FuelTypeValues.map((fuelType) => {
    const baseWeekly = latestPh.find((x: any) => x.fuelType === fuelType)?.weeklyChange ?? 0;
